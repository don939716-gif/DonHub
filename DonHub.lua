--[[
    DonHub - Parry Timing Logger V4 (Mob Specific)
    Game: The Forge
    Author: Don
    
    CHANGELOG:
    - Now logs [Mob Name] + [Sound Name].
    - Strips random numbers from Mob names automatically.
    - Allows different timings for the same sound on different mobs.
]]
...
local OldUI = CoreGui:FindFirstChild("DonHub_ParryLogger_V4")
if OldUI then OldUI:Destroy() end

--// UI CREATION \\--
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "DonHub_ParryLogger_V4"
...
MainFrame.Size = UDim2.new(0, 350, 0, 400) -- Slightly wider for long names
...
Title.Text = "Parry Logger v4 (Mob Specific)"
...
local function UpdateUI(MobName, SoundName, Delay)
    ...
    if LoggedData[Key] then
        ...
        Label.TextSize = 11 -- Slightly smaller to fit long names
...
local function MonitorMob(Mob)
    ...
    local Conn = Mob.HumanoidRootPart.ChildAdded:Connect(function(Child)
        if Child:IsA("Sound") then
            -- STRICT FILTER: Only allow sounds with "Swing" in the name
